...
def unique_houses(filename):
    ...
    >>> unique_houses("cohort_data.txt") == set(['Gryffindor', 'Hufflepuff', "Dumbledore's Army", 'Ravenclaw', 'Slytherin'])
    True
    ...
    #iterate through file, .split(|)(print check), .add[2] to houses
    for line in open_file:
        line = line.rstrip()
        cohort_list = line.split('|')
        houses.add(cohort_list[2])
...
def sort_by_cohort(filename):
    ...
    # Code goes here
    #iterate through file, split(|), if list[-1] == g append into ghost
    open_file = open(filename)
    for line in open_file:
        line = line.rstrip()
        cohort_list = line.split('|')
...
def hogwarts_by_house(filename):
    ...
    for line in open_file:
        ...
        if cohort_list[-1] == "G":
            ghosts.append(cohort_list[1])
        elif cohort_list[-1] == "I":
            instructors.append(cohort_list[1])
        elif cohort_list[2] == "Gryffindor":
            gryffindor.append(cohort_list[1])
        elif cohort_list[2] == "Hufflepuff":
            hufflepuff.append(cohort_list[1])
        elif cohort_list[2] == "Ravenclaw":
            ravenclaw.append(cohort_list[1])
        elif cohort_list[2] == "Slytherin":
            slytherin.append(cohort_list[1])
        elif cohort_list[2] == "Dumbledore's Army":
            dumbledores_army.append(cohort_list[1])
